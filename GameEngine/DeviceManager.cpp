...
namespace vkw {
	VkResult DeviceManager::initialize(const VkPhysicalDevice& physicalDevice, const std::vector<std::uint32_t>& queueFamilyIndices, const std::vector<std::vector<float>>& queuePriorities, const std::vector<const char*>& extensions, const std::vector<const char*>& layers) {
		...
		if (result == VK_SUCCESS) {
			device_ = std::make_shared<Device>(device, allocator_);
			return result;
		}
		else return result;
	}
